...
# This program generates the position of a thrown object being acted on by gravity on Mercury
# Run the program from the Terminal by entering "python3 mercury.py"

def mercury(pi, vi, a, t):
	"""The mercury function follows a physics equation which computes the position (pf) of
	 an object given its initial position (pi), initial velocity (vi), the acceleration acting
	 on the object (a), and the time duration over which the object is thrown (t) on Mercury.  
	 This function takes the parameters pi, vi, a, and t, and returns the value for the 
	 object's trajectory on Mercury (the pf value when given pi, vi, a, and t values as its 
	 parameters)."""
	vi = (0.384*vi)	#velocity on Mercury is 0.384x velocity on Earth
	a = (0.378*a)	#acceleration on Mercury is 0.378x acceleration on Earth
	pf = (pi + (vi*t) + (.5*a*(t**2))) 
	return(pf) #saves the value of the above equation

testMercury = mercury(2, 11, -10, 0.5) #calculates pf for pi=2, vi=2, a=-10, t=0.5 and sets resulting value to the variable testMercury

def computeAndOutput (pi, vi, a, t):
	"""The computeAndOutput function computes the pf (position) value using the mercury
	...
	the mercury.csv file."""
	y = mercury(pi, vi, a, t) #sets the return value of mercury with the parameters (pi, vi, a, t) to the variable y
	pf = (pi + (vi*t) + (.5*a*(t**2))) #the physical equation for trajectory
	fp = open( 'mercury.csv', 'a' ) #opens a file named "mercury.csv" and prepares to add lines of data to the file
...
def clearfile():
	...
	fp = open( 'mercury.csv', 'w' ) #writes the file by creating it from scratch and deleting any pre existing content
	fp.close() #closes the file
